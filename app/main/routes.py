...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(_('Ваше сообщение опубликовано!'))
    ...
    return render_template('index.html', title=_('Главная страница'), form=form, posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def explore():
    ...
    prev_url = url_for('main.index', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('index.html', title=_('Исследование'), posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def edit_profile():
    ...
    return render_template('edit_profile.html', title=_('Редактировать профиль'), form=form)
