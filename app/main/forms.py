...
class EditProfileForm(FlaskForm):
    ...
    def validate_username(self, username):
        if username.data != self.original_username:
            ...
            if user is not None:
                raise ValidationError(_l('Пожалуйста используйте другое имя пользователя.'))

class PostForm(FlaskForm):
    post = TextAreaField(_l('Скажите что-нибудь'), validators=[DataRequired(), Length(min=1, max=140)])
    submit = SubmitField(_l('Отправить'))
